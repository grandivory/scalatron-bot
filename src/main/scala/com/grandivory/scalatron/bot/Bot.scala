...
object Bot {
    ...
    case Some(React(
      ...
      role match {
        ...
        case ROLE_GATHERER =>
          ...
          val spawnCommand =
            if (generation > 2 || currentEnergy < 100) None
          ...
          val missileCommand = spawnCommand match {
            case None if generation <= 2 && currentEnergy >= 100 =>
  ...
  def nearestSafeCell(cell: RelativePosition, view: View): Option[RelativePosition] = {
    view.objectsInView.keys.toList.sorted.find {
  ...
  def shortestPathTo(cell: RelativePosition, view: View): Option[List[Direction]] = {
    ...
    def nextNodes(pathSoFar: Vector[Direction], position: RelativePosition, visited: Set[RelativePosition]):
      ...
      Direction.allPossibleDirections map { direction: Direction =>
        ...
        case (_, newPosition: RelativePosition) =>
          !visited.contains(newPosition) && okToMove(view.objectAt(newPosition))
      }
    }
